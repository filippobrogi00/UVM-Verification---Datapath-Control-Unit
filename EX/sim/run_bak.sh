...
# Use case 3: ./run.sh 1000 faultsim fault_list.txt   run fault simulation with 1000 sequence items and fault list file

if [[ $# -ne 1 && $# -ne 3 ]]; then
  echo "Usage: $0 <number of sequence items per sequence to generate> [<sim|faultsim> <fault_list_file>]"
  exit
fi
  ...
  if compgen -G "$dir"/*."$ext" >/dev/null; then
    ...
    if [[ "$filetype" == "SYSTEMVERILOG" ]]; then
      ...
      if [[ "$compilation" == "TESTBENCH" ]]; then
        ...
        if [[ $# -eq 3 ]]; then 
          $compiler="$compiler $COMPILE_FAULT_OPTIONS" $SV_COMPILE_LIST
        fi
...
######################################################
# Fault injection variable
SIM_FAULT_INJECTION_CAMPAIGN="" # Fault-free simulation default value
FAULT_LIST_FILE=""
EXIT_AFTER_UVM_ERROR="" # Fault-free simulation default value

# Set variables from command line argument
if [[ $# -eq 3 && "$2" == "faultsim" ]]; then 
  FAULT_LIST_FILE="$3"
  SIM_FAULT_INJECTION_CAMPAIGN="+define+FAULT_INJECTION_CAMPAIGN"
  EXIT_AFTER_UVM_ERROR="+UVM_MAX_QUICK_COUNT=1" # Exit simulation after first UVM_ERROR
fi
...
# Fault simulation options grouped together
COMPILE_FAULT_OPTIONS="$SIM_FAULT_INJECTION_CAMPAIGN $EXIT_AFTER_UVM_ERROR"
