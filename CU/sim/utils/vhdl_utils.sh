#### @name: get_vhdl_entity
#### @inputs:
####   VHDL file
#### @return:
####   Returns the VHDL entity name
#### @desc:
####   Extracts the VHDL entity name from the given file
get_vhdl_entity() {
  local file="$1"

  sed -n '
    s/--.*$//            # strip VHDL comments
    /^[[:space:]]*entity[[:space:]]\+/I {
      s/^[[:space:]]*entity[[:space:]]\+\([^[:space:]]\+\)[[:space:]]\+is.*/\1/Ip
      q
    }
  ' "$file"
}
