...
SV_COMPILE_LIST="$TB_DIR/pkg_constants.sv $TB_DIR/Iface_CU.sv $TB_DIR/Module_CU_Wrapper.sv $TB_DIR/Module_topTestbench.sv"
...
if [[ postsyn_sim_enabled -eq 1 ]]; then 
  ...
  if [[ $faultsim_enabled -eq 1 ]]; then 
    ...
    -t $SIM_TIMESCALE $SIM_SEQITEMS $SIM_OPTIONS \
    -do "$VSIM_RUN_AND_REPORT_COV" \
    -sdftyp /${ENVVAR_TB_TOPLEVEL_NAME}/${wrapper_inst}/${dut_inst}=${SYN_DIR}/DP_CU.sdf # specify SDF file
