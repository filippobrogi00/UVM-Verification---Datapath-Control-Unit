...
source ./utils/systemverilog_utils.sh # get_systemverilog_testbench_module()
...
TB_DIR="tb"
SYN_DIR="syn"
COV_BASEDIR="$SIM_DIR/coverage_syn"
COV_DIR="$COV_BASEDIR/text"
COV_HTML_DIR="$COV_BASEDIR/html"
...
mkdir -p $SRC_DIR
mkdir -p $TB_DIR
# # Recreate synthesis dir if recompile flag is set
# if [[ "$1" == "--synth" ]]; then
#   shift
#   [ -d "$SYN_DIR" ] && rm -rf "$SYN_DIR"
#   mkdir -p $SYN_DIR
#   cp .synopsys_dc.setup $SYN_DIR/
# fi

# Reset coverage dirs each time
[ -d "$COV_BASEDIR" ] && rm -rf "$COV_BASEDIR"
[ -d "$COV_DIR" ] && rm -rf "$COV_DIR"
[ -d "$COV_HTML_DIR" ] && rm -rf "$COV_HTML_DIR"
mkdir -p $COV_BASEDIR
mkdir -p $COV_DIR
# DO NOT create COV_HTML_DIR or vcover will complain if it already exists
...
cov_print() {
  ...
  # If functional coverage is required, create the file from transcript
  filename=""
  if [[ "$1" == "Functional" ]]; then
    # Extract functional coverage from transcript
    grep "^# Functional Coverage:" transcript > "$COV_DIR/cov_functional.txt"
    # Set filename to the created file
    filename="$COV_DIR/cov_functional.txt"
  else
    filename="$1"
  fi 
  ...
  if [[ $cov_type == "Functional" ]]; then
    total_cov_line="$(grep "^# Functional Coverage: " $COV_DIR/cov_functional.txt)"
  else
...
# Synthesize the design if synthesis flag is set
# if [[ "$1" == "--synth" ]]; then
print_green "############ SYNTHESIS: ############ "
cd "$SYN_DIR"
[ -d "work" ] && rm -rf "work" && mkdir -p "work"
export SRC_DIR
export SYN_DIR
dc_shell-xg-t -f $SIM_DIR/synthesis.tcl
# fi

# Compile Gate level netlist
colorize vlog -timescale=1ns/1ps -work work /eda/dk/nangate45/verilog/NangateOpenCellLibrary.v
colorize vlog -timescale=1ns/1ps -work work ${SYN_DIR}/DP_IFID.v
...
######################################################
print_green "############ POST-SYNTHESIS SIMULATION: ############ "
########## vsim parameters ##########
SIM_TIMESCALE="10ps"
...
##################################################
# Create HTML Report from .ucdb coverage file
vcover report -details -html $COV_DB_NAME -output "$COV_HTML_DIR"
...
[ -f coverage.ucdb ] && rm coverage.ucdb
[ -d work ] && rm -rf work
[ -f coverage.ucdb ] && rm coverage.ucdb
