...
class Class_MEMWB_Environment extends uvm_env;
  ...
  virtual function void connect_phase(uvm_phase phase);
    ...
    // For loop kind of looses its meaning due to clocking block,
    // but if sampling of inputs and outputs occurs at the same clock cycle
    // on the monitor's side and the monitor is well modelled, 
    // each monitor will monitor one stage of the pipeline.
    // And each monitor will observe the control signals generated by
    // the same stimuli, even in the presence of pipelining.
    ...
    memwb_agent.memwb_monitor[i].analysis_port.connect(
